...
def simple_normalize(train_data=None, val_data=None, mean_x=None, std_x=None):
    if train_data is not None:
        res = 1
        mean_x = train_data.x.reshape(-1, 4, 128 * res, 128 * res).mean(
            (0, 2, 3), keepdim=True
        )
        std_x = train_data.x.reshape(-1, 4, 128 * res, 128 * res).std(
            (0, 2, 3), keepdim=True
        )
        std_x[std_x == 0] = 1
    ...
    def __init__(
            ...
            with h5py.File(root_path, "r") as h5_file:
                ...
                if len(data.shape) == 5:
                    ...
                    self.grid = torch.tensor(
                        grid[::reduced_resolution], dtype=torch.float
                    ).unsqueeze(-1)
        ...
        if len(self.data[..., 0, :].shape) == 3:
            ...
            if self.x.shape[2] < 4:
                ...
                self.x = torch.cat(
                    [self.x] + (4 - self.x.shape[2]) * [torch.zeros_like(self.x)], 2
                )

            self.x = self.x.reshape(ori_shape[0], ori_shape[1], 4, 1, ori_shape[2])
            self.x = self.x.broadcast_to(
                ori_shape[0], ori_shape[1], 4, ori_shape[2], ori_shape[2]
            )
            ...
            self.grid = self.grid.unsqueeze(-2).broadcast_to(
                self.grid.shape[0], self.grid.shape[1], self.grid.shape[1]
            )
            ...
            if self.x.shape[-1] < 4:
                ...
                self.x = torch.cat(
                    [self.x]
                    + (4 - self.x.shape[-1]) * [torch.zeros_like(self.x[..., :1])],
                    -1,
                )

            self.x = self.x.permute(0, 1, 4, 2, 3).reshape(
                self.x.shape[0], self.x.shape[1], -1, self.x.shape[2], self.x.shape[3]
            )
