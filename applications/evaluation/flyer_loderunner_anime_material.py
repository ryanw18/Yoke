...
DEFAULT_TIMESTEP_DELTA = 0.05  # us, expected flyerplate dump cadence
...
def flyer_density_indices(default_vars: list[str]) -> list[int]:
    ...
    for idx, field in enumerate(default_vars):
        if field.startswith("density_") and field != "density_Air":
...
if __name__ == "__main__":
    ...
    if len(npz_list) >= 2:
        first_time = scalarPVIarray(npz_list[0], "sim_time")
        second_time = scalarPVIarray(npz_list[1], "sim_time")
        timestep_delta = float(second_time - first_time)
        if timestep_delta <= 0.0:
            timestep_delta = DEFAULT_TIMESTEP_DELTA
    else:
        timestep_delta = DEFAULT_TIMESTEP_DELTA
