...
# I commented this out to see if they increase the coverage at all. I suspect they don't
#
#
# @patch("yoke.datasets.diffusion_dataset.volfrac_density", side_effect=mock_volfrac_density)
# @patch(
...
# )
# @patch("pathlib.Path.is_file", return_value=True)
# def test_diffusion_temporal_lead_time_calculation(
#     mock_is_file: MagicMock,
#     mock_npz_load: MagicMock,
#     mock_LSCread_npz_NaN: MagicMock,
#     mock_volfrac_density: MagicMock,
...
#     # Mock the random number generator to return predictable values
#     with patch.object(
#         diffusion_temporal_dataset.rng, "integers", side_effect=[2, 5]
#     ):  # seqLen=2, startIDX=5
#         x, y_tau, noise, lead_time, tau = diffusion_temporal_dataset[0]
#
#         # Lead time should be 0.25 * (endIDX - startIDX) = 0.25 * 2 = 0.5
#         expected_lead_time = 0.25 * 2
#         assert pytest.approx(lead_time.item(), rel=1e-6) == expected_lead_time
#
#
# @patch("yoke.datasets.diffusion_dataset.volfrac_density", side_effect=mock_volfrac_density)
# @patch(
...
# )
# @patch("pathlib.Path.is_file", return_value=True)
# def test_diffusion_temporal_forward_diffusion_applied(
#     mock_is_file: MagicMock,
#     mock_npz_load: MagicMock,
#     mock_LSCread_npz_NaN: MagicMock,
#     mock_volfrac_density: MagicMock,
#     diffusion_temporal_dataset: DiffusionLSC_temporal_DataSet,
# ) -> None:
#     """Test that forward diffusion is applied correctly."""
#     # Mock the noise schedule's forward_diffusion method
#     mock_forward_diffusion = MagicMock(
#         return_value=(
#             torch.ones((1, 8, 10, 10)) * 0.5,  # y_tau
#             torch.ones((1, 8, 10, 10)) * 0.3,  # noise
#         )
#     )
#
#     with patch.object(
#         diffusion_temporal_dataset.noise_schedule,
#         "forward_diffusion",
#         mock_forward_diffusion,
#     ):
#         x, y_tau, noise, lead_time, tau = diffusion_temporal_dataset[0]
#
#         # Check that forward_diffusion was called
#         assert mock_forward_diffusion.called
#
#         # Check that the returned values match the mocked values (after squeeze)
#         assert torch.allclose(y_tau, torch.ones((8, 10, 10)) * 0.5)
#         assert torch.allclose(noise, torch.ones((8, 10, 10)) * 0.3)
#
#
# def test_diffusion_temporal_index_rotation(
#     diffusion_temporal_dataset: DiffusionLSC_temporal_DataSet,
# ) -> None:
#     """Test that index rotation works correctly."""
#     # Test that indices beyond Nsamples are rotated
#     assert diffusion_temporal_dataset.Nsamples == 3
#
#     # Index 3 should rotate to 0, index 4 to 1, etc.
#     # We can't directly test __getitem__ without mocking file operations,
#     # but we can verify the rotation logic
#     for idx in [3, 4, 5, 6]:
#         rotated_idx = idx % diffusion_temporal_dataset.Nsamples
#         assert rotated_idx in [0, 1, 2]
#
#
# @patch("yoke.datasets.diffusion_dataset.volfrac_density", side_effect=mock_volfrac_density)
# @patch(
...
# @patch("numpy.load")
# @patch("pathlib.Path.is_file", return_value=True)
# def test_diffusion_temporal_npz_files_closed(
#     mock_is_file: MagicMock,
#     mock_npz_load: MagicMock,
#     mock_LSCread_npz_NaN: MagicMock,
#     mock_volfrac_density: MagicMock,
#     diffusion_temporal_dataset: DiffusionLSC_temporal_DataSet,
# ) -> None:
#     """Test that NPZ files are properly closed after loading."""
#     mock_start_npz = MockNpzFile({"dummy_field": np.ones((10, 10))})
#     mock_end_npz = MockNpzFile({"dummy_field": np.ones((10, 10))})
#
#     mock_start_npz.close = MagicMock()
#     mock_end_npz.close = MagicMock()
#
#     mock_npz_load.side_effect = [mock_start_npz, mock_end_npz]
#
#     x, y_tau, noise, lead_time, tau = diffusion_temporal_dataset[0]
#
#     # Verify that close was called on both NPZ files
#     mock_start_npz.close.assert_called_once()
#     mock_end_npz.close.assert_called_once()
#
#
# @patch("pathlib.Path.is_file", return_value=True)
# @patch("numpy.load", side_effect=Exception("Start file load error"))
# def test_diffusion_temporal_start_file_load_error(
#     mock_npz_load: MagicMock,
#     mock_is_file: MagicMock,
#     diffusion_temporal_dataset: DiffusionLSC_temporal_DataSet,
# ) -> None:
#     """Test error handling when start file fails to load."""
#     with pytest.raises(Exception, match="Start file load error"):
#         diffusion_temporal_dataset[0]
#
#
# @patch("pathlib.Path.is_file", return_value=True)
# @patch("numpy.load")
# def test_diffusion_temporal_end_file_load_error(
#     mock_npz_load: MagicMock,
#     mock_is_file: MagicMock,
#     diffusion_temporal_dataset: DiffusionLSC_temporal_DataSet,
# ) -> None:
#     """Test error handling when end file fails to load and start file is closed."""
#     mock_start_npz = MockNpzFile({"dummy_field": np.ones((10, 10))})
#     mock_start_npz.close = MagicMock()
#
#     mock_npz_load.side_effect = [
#         mock_start_npz,
#         Exception("End file load error"),
#     ]
#
#     with pytest.raises(Exception, match="End file load error"):
#         diffusion_temporal_dataset[0]
#
#     # Verify start file was closed even though end file failed
#     mock_start_npz.close.assert_called_once()
